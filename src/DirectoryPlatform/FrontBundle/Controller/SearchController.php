...
use DirectoryPlatform\AppBundle\Entity\Search;

class SearchController extends Controller
{
    /**
     * @Route("/account/searches", name="search")
     */
    ...
    public function createAction(Request $request, $entity)
    {
        ...
        if (!empty($referer) && is_array(explode('?', $referer)) && count(explode('?', $referer)) > 1) {            
            $query = '';
            ...
            try {
                ...
                $em->persist($search);
                $em->flush();
                $this->addFlash('success', $this->get('translator')->trans('Search query has been successfully saved.'));
            } catch (\Exception $e) {
     ...
     * @ParamConverter("search", class="DirectoryPlatform\AppBundle\Entity\Search")
     */
